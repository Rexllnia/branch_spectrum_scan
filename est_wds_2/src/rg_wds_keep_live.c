...
void rg_wds_send_keep_date_cpe()
{
	...
	rg_send_raw_date(rg_ath_info_t.ath_wsd_name, 42 + sizeof(struct mac_ip_udp_wds_packet) + sizeof(struct wds_keeplive_packet),buf,rg_pair_info_heap_t->mac);
}
...
void rg_wds_get_keep_date(char *data,int len) {
    if (rg_pair_info_heap_t == NULL ) {
        DEBUG("now wlanconfig list is null");
        return;
    }
...
void rg_wds_keep_date_send_ap(struct pair_dev_ath_info *p) {
	...
	memcpy(buf + sizeof(struct mac_ip_udp_wds_packet),&keep_live_date,sizeof(struct wds_keeplive_packet));
    /* 42代表：为KEEPALIVE报文多增加42字节，用于兼容旧版本多发42字节问题 */
	rg_send_raw_date(rg_ath_info_t.ath_wsd_name, 42 + sizeof(struct mac_ip_udp_wds_packet) + sizeof(struct wds_keeplive_packet),buf,p->mac);
}
