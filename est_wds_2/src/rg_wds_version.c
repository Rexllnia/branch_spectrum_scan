...
void rg_wds_soft_version_cpe_send() {
	...
	for (i = 0;i < 5;i++) {
		rg_send_raw_date(rg_ath_info_t.ath_wsd_name,data_len,buf,mac);
	}
}
...
void rg_wds_soft_version_ap_send() {
    ...
    while (p) {
        for (i = 0;i < 5;i++) {
            rg_send_raw_date(rg_ath_info_t.ath_wsd_name,data_len,buf,p->mac);
        }
    ...
    while (p) {
        if (memcmp(mac,p->mac,6) == 0) {
            if (memcmp(p->pair_dev_info_t.software_version,softversion_date,sizeof(p->pair_dev_info_t.software_version)) != 0) {
                ...
                DEBUG("software_version %s",p->pair_dev_info_t.software_version);
                break;
            }
        }
        p = p->next;
