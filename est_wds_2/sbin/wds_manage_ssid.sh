...
ap_cpe_switch()
{
    tmp_mode=`est_wds_gpio_read $mode_value $dev_type`
...
ap_mode=`est_wds_gpio_read $mode_value $dev_type`
while true
do
    # 如果原始管理ssid被disable，就不进入隐藏流程
    if [ $(uci -q get wireless.@wifi-iface[1].disabled) -eq 1 ]; then
        # 将倒计时隐藏的参数清零，重新开始倒计时
        timer_start=0
        sleep 5
        continue
    ...
    # 如果是EST300，lock_value值默认为LOCK
    if [ "$dev_type" == "p2p" -a $lock_value -eq 0 ]; then
        lock_value=1
    fi
    if [ ${lock_value} = "1" ]; then
        ...
        if [ ${timer_start} -eq 0 ]; then
            ...
            do
                ...
                # EST300设备不判断。如果检测到lock状态发生变化，停止倒计时
                if [ "$dev_type" != "p2p" ]; then
                    lock_value=`est_wds_gpio_read $lock_led $dev_type`
                    [ $lock_value -eq 0 ] && timer_start=0 && break
                fi
