...
wds_status()
{
    ...
    if [ "$ap_status" == "0" ] ;then
        ...
        led_send_message "rssi3;wds_off" > /dev/null
        return
    fi
    ...
    if [ "$ap_status" == "1" ] ;then
        ...
        if [ $assoc_item_tmp -lt -78 ] ;then
            ...
            led_send_message "rssi1;wds_on" > /dev/null
            led_send_message "rssi2;wds_on" > /dev/null
            led_send_message "rssi3;wds_on" > /dev/null
        fi
    fi
}
...
wds_lock_led()
{
    ...
    now_value=`est_wds_gpio_read $lock_led $dev_type`
...
{
    ap_mode=`est_wds_gpio_read $ap_value $dev_type`
    lock_value=`est_wds_gpio_read $lock_led $dev_type`
    wds_ssid=$(uci -q get wireless.@wifi-iface[0].ssid)
    if [ "$wds_ssid" != "$DEF_SSID" ] && [ $lock_value -eq 1 ]; then
        if [ $ap_mode -eq 1 ] && [ -n "$(uci -q get wireless.@wifi-iface[0].maclist)" ]; then
            ...
            uci commit wireless
        elif [ $ap_mode -eq 0 ] && [ -n "$(uci -q get wireless.@wifi-iface[0].bssid)" ]; then
...
# 获取设备能力值dev_type，判断是否为EST300
dev_type=$(jq -r '.wireless.wds_type' $DEVINFO_JSON_FILE)
while true
do
    wds_status
    wds_lock_led
