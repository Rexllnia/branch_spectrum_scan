...
char rg_wds_json_first(char *file_name,char *option,char *buf,char len) {
	...
	if (obj_all_p_1 == NULL) {
        GPIO_DEBUG("can not open json file %s",file_name);
        ret = FAIL;
        goto realese;
	}

    obj_all_p = json_object_object_get(obj_all_p_1, option);
    if (is_error(obj_all_p)) {
        GPIO_DEBUG("can not find option %s",option);
        ret = FAIL;
        goto realese;
    }
...
realese:
    ...
    if (obj_all_p_1) {
	    json_object_put(obj_all_p_1); //coredump
    }
...
char rg_wds_json_second_cmp(char *file_name,char *first_option,char *second_option,char *cmp_src,char *buf,char len) {
    ...
    if(obj_all_p == NULL) {
        GPIO_DEBUG("can not find option %s",first_option);
        ret = FAIL;
        goto realese;
    }
	...
	for(i = 0; i < json_object_array_length(obj_all_p); i++) {
        ...
        if(memcmp(cmp_src,json_object_to_json_string(cmp_des) + 1,strlen(json_object_to_json_string(cmp_des)) - 2)) {
            GPIO_DEBUG("cmp_src %s ",json_object_to_json_string(cmp_des));
        }
    }
...
char rg_wds_first_set(char *file_name, char *first_option, char *buf) {
    ...
    json_object_object_add(obj_all_p, first_option, json_object_new_string(buf));
    rg_wds_misc_clear_file(file_name);
    const char *str = json_object_to_json_string(obj_all_p);
    int fd;
...
char rg_wds_second_set(char *file_name,char *first_option,char *second_option,char *buf) {
    ...
    GPIO_DEBUG("obj_all_p %s second_option %s buf %s %d",json_object_to_json_string(obj_all_p),second_option,buf,json_object_array_length(obj_all_p));
	for(i = 0; i < json_object_array_length(obj_all_p); i++) {
		json_object *section= json_object_array_get_idx(obj_all_p, i);
